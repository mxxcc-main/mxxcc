...
// Tests the go-ccmchain to Alccm chainspec conversion for the Stureby testnet.
func TestAlccmSturebyConverter(t *testing.T) {
	blob, err := ioutil.ReadFile("testdata/stureby_gccm.json")
	if err != nil {
	...
	spec, err := newAlccmGenesisSpec("stureby", &genesis)
	if err != nil {
		t.Fatalf("failed creating chainspec: %v", err)
	}

	expBlob, err := ioutil.ReadFile("testdata/stureby_alccm.json")
	if err != nil {
		t.Fatalf("could not read file: %v", err)
	}
	expspec := &alccmGenesisSpec{}
...
// Tests the go-ccmchain to Parity chainspec conversion for the Stureby testnet.
