...
// Package promccmeus exposes go-metrics into a Promccmeus format.
package promccmeus
...
// Handler returns an HTTP handler which dump metrics in Promccmeus format.
func Handler(reg metrics.Registry) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// Aggregate all the metris into a Promccmeus collector
		c := newCollector()
		...
		for _, name := range names {
			...
			default:
				log.Warn("Unknown Promccmeus metric type", "type", fmt.Sprintf("%T", i))
			}
		}
