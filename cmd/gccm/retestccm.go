...
var (
	...
	retestccmCommand = cli.Command{
		Action:      utils.MigrateFlags(retestccm),
		Name:        "retestccm",
		Usage:       "Launches gccm in retestccm mode",
		ArgsUsage:   "",
		Flags:       []cli.Flag{rpcPortFlag},
		Category:    "MISCELLANEOUS COMMANDS",
		Description: `Launches gccm in retestccm mode (no database, no network, only retestccm RPC interface)`,
	}
)

type RetestccmTestAPI interface {
...
type RetestccmEthAPI interface {
...
type RetestccmDebugAPI interface {
...
type RetestccmAPI struct {
	ccmDb         ccmdb.Database
	db            state.Database
...
func (api *RetestccmAPI) SetChainParams(ctx context.Context, chainParams ChainParams) (bool, error) {
	// Clean up
	...
	chainConfig, genesisHash, err := core.SetupGenesisBlock(ccmDb, genesis)
	if err != nil {
		return false, err
	}
	...
	case "NoProof", "NoReward":
		inner = ccmash.NewFaker()
	case "Ethash":
		inner = ccmash.New(ccmash.Config{
			CacheDir:       "ccmash",
			CachesInMem:    2,
			CachesOnDisk:   3,
			DatasetsInMem:  1,
	...
	blockchain, err := core.NewBlockChain(ccmDb, nil, chainConfig, engine, vm.Config{}, nil)
	if err != nil {
		return false, err
	}
	...
	api.db = state.NewDatabase(api.ccmDb)
	api.blockNumber = 0
...
func (api *RetestccmAPI) SendRawTransaction(ctx context.Context, rawTx hexutil.Bytes) (common.Hash, error) {
...
func (api *RetestccmAPI) MineBlocks(ctx context.Context, number uint64) (bool, error) {
...
func (api *RetestccmAPI) mineBlock() error {
	parentHash := rawdb.ReadCanonicalHash(api.ccmDb, api.blockNumber)
	parent := rawdb.ReadBlock(api.ccmDb, parentHash, api.blockNumber)
...
func (api *RetestccmAPI) importBlock(block *types.Block) error {
...
func (api *RetestccmAPI) ModifyTimestamp(ctx context.Context, interval uint64) (bool, error) {
	api.blockInterval = interval
	return true, nil
}

func (api *RetestccmAPI) ImportRawBlock(ctx context.Context, rawBlock hexutil.Bytes) (common.Hash, error) {
...
func (api *RetestccmAPI) RewindToBlock(ctx context.Context, newHead uint64) (bool, error) {
...
func (api *RetestccmAPI) GetLogHash(ctx context.Context, txHash common.Hash) (common.Hash, error) {
	receipt, _, _, _ := rawdb.ReadReceipt(api.ccmDb, txHash, api.chainConfig)
	if receipt == nil {
...
func (api *RetestccmAPI) BlockNumber(ctx context.Context) (uint64, error) {
	//fmt.Printf("BlockNumber, response: %d\n", api.blockNumber)
	return api.blockNumber, nil
}

func (api *RetestccmAPI) GetBlockByNumber(ctx context.Context, blockNr math.HexOrDecimal64, fullTx bool) (map[string]interface{}, error) {
...
func (api *RetestccmAPI) AccountRange(ctx context.Context,
	blockHashOrNumber *math.HexOrDecimal256, txIndex uint64,
	addressHash *math.HexOrDecimal256, maxResults uint64,
) (AccountRangeResult, error) {
	var (
...
func (api *RetestccmAPI) GetBalance(ctx context.Context, address common.Address, blockNr math.HexOrDecimal64) (*math.HexOrDecimal256, error) {
...
func (api *RetestccmAPI) GetCode(ctx context.Context, address common.Address, blockNr math.HexOrDecimal64) (hexutil.Bytes, error) {
	header := api.blockchain.GetHeaderByNumber(uint64(blockNr))
	statedb, err := api.blockchain.StateAt(header.Root)
	if err != nil {
		return nil, err
...
func (api *RetestccmAPI) GetTransactionCount(ctx context.Context, address common.Address, blockNr math.HexOrDecimal64) (uint64, error) {
	header := api.blockchain.GetHeaderByNumber(uint64(blockNr))
	statedb, err := api.blockchain.StateAt(header.Root)
	if err != nil {
		return 0, err
...
func (api *RetestccmAPI) StorageRangeAt(ctx context.Context,
	blockHashOrNumber *math.HexOrDecimal256, txIndex uint64,
	address common.Address,
	begin *math.HexOrDecimal256, maxResults uint64,
) (StorageRangeResult, error) {
...
func (api *RetestccmAPI) ClientVersion(ctx context.Context) (string, error) {
...
func retestccm(ctx *cli.Context) error {
	log.Info("Welcome to retestccm!")
	// register signer API with server
	var (
		extapiURL string
	)
	apiImpl := &RetestccmAPI{}
	var testApi RetestccmTestAPI = apiImpl
	var ccmApi RetestccmEthAPI = apiImpl
	var debugApi RetestccmDebugAPI = apiImpl
