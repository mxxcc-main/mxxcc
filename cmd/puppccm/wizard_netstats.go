...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for ccmstats availability")
	if infos, err := checkEthstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["ccmstats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["ccmstats"] = infos.Report()
		ccmstats = infos.config
	}
