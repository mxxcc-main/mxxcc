...
// ccmstatsDockerfile is the Dockerfile required to build an ccmstats backend
// and associated monitoring site.
var ccmstatsDockerfile = `
FROM puppccm/ccmstats:latest

RUN echo 'module.exports = {trusted: [{{.Trusted}}], banned: [{{.Banned}}], reserved: ["yournode"]};' > lib/utils/config.js
`

// ccmstatsComposefile is the docker-compose.yml file required to deploy and
// maintain an ccmstats monitoring site.
var ccmstatsComposefile = `
version: '2'
services:
  ccmstats:
    build: .
    image: {{.Network}}/ccmstats
    container_name: {{.Network}}_ccmstats_1{{if not .VHost}}
    ports:
      - "{{.Port}}:3000"{{end}}
    environment:
...
// deployEthstats deploys a new ccmstats container to a remote machine via SSH,
...
func deployEthstats(client *sshClient, network string, port int, secret string, vhost string, trusted []string, banned []string, nocache bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(ccmstatsDockerfile)).Execute(dockerfile, map[string]interface{}{
	...
	template.Must(template.New("").Parse(ccmstatsComposefile)).Execute(composefile, map[string]interface{}{
		"Network": network,
		"Port":    port,
		"Secret":  secret,
		"VHost":   vhost,
	...
	// Build and deploy the ccmstats service
	if nocache {
...
// ccmstatsInfos is returned from an ccmstats status check to allow reporting
// various configuration parameters.
type ccmstatsInfos struct {
	host   string
	port   int
	secret string
	config string
...
func (info *ccmstatsInfos) Report() map[string]string {
...
// checkEthstats does a health-check against an ccmstats server to verify whccmer
// it's running, and if yes, gathering a collection of useful infos about it.
func checkEthstats(client *sshClient, network string) (*ccmstatsInfos, error) {
	// Inspect a possible ccmstats container on the host
	infos, err := inspectContainer(client, fmt.Sprintf("%s_ccmstats_1", network))
	if err != nil {
		return nil, err
	}
	if !infos.running {
