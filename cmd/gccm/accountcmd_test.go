...
func TestAccountListEmpty(t *testing.T) {
	gccm := runGccm(t, "account", "list")
	gccm.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gccm := runGccm(t, "account", "list", "--datadir", datadir)
	defer gccm.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	gccm := runGccm(t, "account", "new", "--lightkdf")
	defer gccm.ExpectExit()
	gccm.Expect(`
...
Your new account is locked with a password. Please give a password. Do not forget this password.
!! Unsupported terminal, password will be echoed.
Passphrase: {{.InputLine "somccming"}}
Repeat passphrase: {{.InputLine "somccming else"}}
...
func TestWalletImport(t *testing.T) {
	gccm := runGccm(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gccm.ExpectExit()
	gccm.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(gccm.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gccm.StderrText(), m) {
...
// https://github.com/ccmchain/go-ccmchain/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gccm.SetTemplateFunc("keypath", func(file string) string {
