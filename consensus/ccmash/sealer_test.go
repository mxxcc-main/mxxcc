...
// Tests whccmer remote HTTP servers are correctly notified of new work.
func TestRemoteNotify(t *testing.T) {
	...
	// Create the custom ccmash engine
	ccmash := NewTester([]string{"http://" + listener.Addr().String()}, false)
	defer ccmash.Close()
	...
	ccmash.Seal(nil, block, nil, nil)
	select {
	case work := <-sink:
		if want := ccmash.SealHash(header).Hex(); work[0] != want {
...
// Tests whccmer stale solutions are correctly processed.
func TestStaleSubmission(t *testing.T) {
	ccmash := NewTester(nil, true)
	defer ccmash.Close()
	api := &API{ccmash}
	...
	for id, c := range testcases {
		for _, h := range c.headers {
			ccmash.Seal(nil, types.NewBlockWithHeader(h), results, nil)
		}
		if res := api.SubmitWork(fakeNonce, ccmash.SealHash(c.headers[c.submitIndex]), fakeDigest); res != c.submitRes {
		...
		case <-time.NewTimer(time.Second).C:
			t.Errorf("case %d fetch ccmash result timeout", id+1)
		}
	}
}
