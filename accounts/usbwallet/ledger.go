...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and Ccmchain address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs an Ccmchain transaction after having the user validate the parameters
...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "Ccmchain app in browser mode", w.failure
	}
	if w.offline() {
		return "Ccmchain app offline", w.failure
	}
	return fmt.Sprintf("Ccmchain app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}

// offline returns whccmer the wallet and the Ccmchain app is offline or not.
//
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if err != nil {
		// Ccmchain app is not running or in browser mode, nothing more to do, return
		if err == errLedgerReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the Ccmchain app's version, will fail prior to v1.0.2
...
// and returning the Ccmchain address located on that derivation path.
...
// ledgerVersion retrieves the current version of the Ccmchain wallet app running
...
// ledgerDerive retrieves the currently active Ccmchain address from a Ledger
...
//   Ccmchain address length | 1 byte
//   Ccmchain address        | 40 bytes hex ascii
...
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (common.Address, error) {
	...
	// Extract the Ccmchain hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return common.Address{}, errors.New("reply lacks address entry")
	}
	hexstr := reply[1 : 1+int(reply[0])]

	// Decode the hex sting into an Ccmchain address and return
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	// Extract the Ccmchain signature and do a sanity validation
	if len(reply) != 65 {
