...
const (
	...
	URN_WANCcmchainnetLinkConfig_1    = "urn:schemas-upnp-org:service:WANCcmchainnetLinkConfig:1"
...
//
// * NewLinkType: allowed values: Ccmchainnet
...
// Return values:
//
// * NewWANAccessType: allowed values: DSL, POTS, Cable, Ccmchainnet
//
...
// WANCcmchainnetLinkConfig1 is a client for UPnP SOAP service with URN "urn:schemas-upnp-org:service:WANCcmchainnetLinkConfig:1". See
// goupnp.ServiceClient, which contains RootDevice and Service attributes which
// are provided for informational value.
type WANCcmchainnetLinkConfig1 struct {
	goupnp.ServiceClient
}

// NewWANCcmchainnetLinkConfig1Clients discovers instances of the service on the network,
// and returns clients to any that are found. errors will contain an error for
// any devices that replied but which could not be queried, and err will be set
// if the discovery process failed outright.
//
// This is a typical entry calling point into this package.
func NewWANCcmchainnetLinkConfig1Clients() (clients []*WANCcmchainnetLinkConfig1, errors []error, err error) {
	var genericClients []goupnp.ServiceClient
	if genericClients, errors, err = goupnp.NewServiceClients(URN_WANCcmchainnetLinkConfig_1); err != nil {
		return
	}
	clients = newWANCcmchainnetLinkConfig1ClientsFromGenericClients(genericClients)
	return
}

// NewWANCcmchainnetLinkConfig1ClientsByURL discovers instances of the service at the given
// URL, and returns clients to any that are found. An error is returned if
// there was an error probing the service.
//
// This is a typical entry calling point into this package when reusing an
// previously discovered service URL.
func NewWANCcmchainnetLinkConfig1ClientsByURL(loc *url.URL) ([]*WANCcmchainnetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsByURL(loc, URN_WANCcmchainnetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANCcmchainnetLinkConfig1ClientsFromGenericClients(genericClients), nil
}

// NewWANCcmchainnetLinkConfig1ClientsFromRootDevice discovers instances of the service in
// a given root device, and returns clients to any that are found. An error is
// returned if there was not at least one instance of the service within the
// device. The location parameter is simply assigned to the Location attribute
// of the wrapped ServiceClient(s).
...
// This is a typical entry calling point into this package when reusing an
// previously discovered root device.
func NewWANCcmchainnetLinkConfig1ClientsFromRootDevice(rootDevice *goupnp.RootDevice, loc *url.URL) ([]*WANCcmchainnetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsFromRootDevice(rootDevice, loc, URN_WANCcmchainnetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANCcmchainnetLinkConfig1ClientsFromGenericClients(genericClients), nil
...
func newWANCcmchainnetLinkConfig1ClientsFromGenericClients(genericClients []goupnp.ServiceClient) []*WANCcmchainnetLinkConfig1 {
	clients := make([]*WANCcmchainnetLinkConfig1, len(genericClients))
	for i := range genericClients {
		clients[i] = &WANCcmchainnetLinkConfig1{genericClients[i]}
	}
	return clients
}
...
// Return values:
//
// * NewCcmchainnetLinkStatus: allowed values: Up, Down
func (client *WANCcmchainnetLinkConfig1) GetCcmchainnetLinkStatus() (NewCcmchainnetLinkStatus string, err error) {
	// Request structure.
	request := interface{}(nil)
	// BEGIN Marshal arguments into request.
	...
	// Response structure.
	response := &struct {
		NewCcmchainnetLinkStatus string
	}{}

	// Perform the SOAP call.
	if err = client.SOAPClient.PerformAction(URN_WANCcmchainnetLinkConfig_1, "GetCcmchainnetLinkStatus", request, response); err != nil {
		return
	}

	// BEGIN Unmarshal arguments from response.

	if NewCcmchainnetLinkStatus, err = soap.UnmarshalString(response.NewCcmchainnetLinkStatus); err != nil {
		return
	}
	// END Unmarshal arguments from response.
	return
