...
enum ccmash_io_rc {
	CCMASH_IO_FAIL = 0,           ///< There has been an IO failure
	CCMASH_IO_MEMO_SIZE_MISMATCH, ///< DAG with revision/hash match, but file size was wrong.
	CCMASH_IO_MEMO_MISMATCH,      ///< The DAG file did not exist or there was revision/hash mismatch
	CCMASH_IO_MEMO_MATCH,         ///< DAG file existed and revision/hash matched. No need to do anything
};
 ...
 * ccmash_full_t
 */
#ifdef CCMASH_PRINT_CRITICAL_OUTPUT
#define CCMASH_CRITICAL(...)							\
	do													\
	{													\
		printf("CCMASH CRITICAL ERROR: "__VA_ARGS__);	\
		printf("\n");									\
		fflush(stdout);									\
	} while (0)
#else
