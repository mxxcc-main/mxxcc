...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"ccmstats", "explorer", "wallet", "faucet"} {
	...
	// If we have ccmstats running, ask whccmer to make the secret public or not
	if w.conf.ccmstats != "" {
		fmt.Println()
		fmt.Println("Include ccmstats secret on dashboard (y/n)? (default = yes)")
