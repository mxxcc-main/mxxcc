...
//   reserving it for go-ccmchain. This would also reduce the memory requirements
//   of Gccm, and thus also GC overhead.
type freezer struct {
...
// HasAncient returns an indicator whccmer the specified ancient data exists
// in the freezer.
...
func (f *freezer) freeze(db ccmdb.KeyValueStore) {
	...
	for {
		...
		// Log somccming friendly for the user
		context := []interface{}{
