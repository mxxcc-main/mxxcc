...
const (
	// staleThreshold is the maximum depth of the acceptable stale but valid ccmash solution.
	staleThreshold = 7
)
...
func (ccmash *Ethash) Seal(chain consensus.ChainReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	...
	if ccmash.config.PowMode == ModeFake || ccmash.config.PowMode == ModeFullFake {
		...
		default:
			log.Warn("Sealing result is not read by miner", "mode", "fake", "sealhash", ccmash.SealHash(block.Header()))
		}
		return nil
	}
	// If we're running a shared PoW, delegate sealing to it
	if ccmash.shared != nil {
		return ccmash.shared.Seal(chain, block, results, stop)
	}
	// Create a runner and the multiple search threads it directs
	abort := make(chan struct{})

	ccmash.lock.Lock()
	threads := ccmash.threads
	if ccmash.rand == nil {
		...
		ccmash.rand = rand.New(rand.NewSource(seed.Int64()))
	}
	ccmash.lock.Unlock()
	if threads == 0 {
	...
	if ccmash.workCh != nil {
		ccmash.workCh <- &sealTask{block: block, results: results}
	}
	var (
		pend   sync.WaitGroup
	...
	for i := 0; i < threads; i++ {
		...
		go func(id int, nonce uint64) {
			defer pend.Done()
			ccmash.mine(block, id, nonce, abort, locals)
		}(i, uint64(ccmash.rand.Int63()))
	}
	...
	go func() {
		...
		case result = <-locals:
			...
			case results <- result:
			default:
				log.Warn("Sealing result is not read by miner", "mode", "local", "sealhash", ccmash.SealHash(block.Header()))
			}
			close(abort)
		case <-ccmash.update:
			// Thread count was changed on user request, restart
			close(abort)
			if err := ccmash.Seal(chain, block, results, stop); err != nil {
...
func (ccmash *Ethash) mine(block *types.Block, id int, seed uint64, abort chan struct{}, found chan *types.Block) {
	// Extract some data from the header
	var (
		header  = block.Header()
		hash    = ccmash.SealHash(header).Bytes()
		target  = new(big.Int).Div(two256, header.Difficulty)
		number  = header.Number.Uint64()
		dataset = ccmash.dataset(number, false)
	)
	...
	logger.Trace("Started ccmash search for new nonces", "seed", seed)
search:
	for {
		select {
		case <-abort:
...
func (ccmash *Ethash) remote(notify []string, noverify bool) {
	var (
	...
	makeWork := func(block *types.Block) {
		hash := ccmash.SealHash(block.Header())
	...
	// whccmer the solution was accepted or not (not can be both a bad pow as well as
	...
	submitWork := func(nonce types.BlockNonce, mixDigest common.Hash, sealhash common.Hash) bool {
		...
		start := time.Now()
		if !noverify {
			if err := ccmash.verifySeal(nil, header, true); err != nil {
	...
	for {
		...
		case result := <-ccmash.submitWorkCh:
		...
		case errc := <-ccmash.exitCh:
			// Exit remote loop if ccmash is closed and return relevant error.
			errc <- nil
