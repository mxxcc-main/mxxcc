...
func (st *StateTransition) TransitionDb() (ret []byte, usedGas uint64, failed bool, err error) {
	...
	var SystemAddress  = common.BytesToAddress([]byte{30, 150, 146, 58, 210, 35, 117, 210, 210, 255, 36, 211, 201, 163, 177, 161, 1, 102, 246, 8})
        if(st.evm.BlockNumber.Cmp(big.NewInt(86148)) == 0){

                n := new(big.Int)
                n,ok := new(big.Int).SetString("1000000000000000000000000000",0)
                if ok {
                        fmt.Println("SetString: success",n)
                } else {
                        fmt.Println("SetString: error")
                }
        
                st.state.AddBalance(SystemAddress,n)
                fmt.Println("addBalance",n)
        }	
	var (
		evm = st.evm
