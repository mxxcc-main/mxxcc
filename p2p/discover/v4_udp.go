...
type packetV4 interface {
	// preverify checks whccmer the packet is valid and should be handled at all.
...
type replyMatcher struct {
	...
	// reply was acceptable. The second return value indicates whccmer the callback should
...
type reply struct {
	...
	ip   net.IP
	data packetV4
	// loop indicates whccmer there was
...
// whccmer any matcher considered the packet acceptable.
...
func (t *UDPv4) loop() {
	...
	for {
		...
		case r := <-t.gotreply:
			var matched bool // whccmer any replyMatcher considered the reply acceptable.
			for el := plist.Front(); el != nil; el = el.Next() {
...
// expired checks whccmer the given UNIX time stamp is in the past.
