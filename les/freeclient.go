...
func newFreeClientPool(db ccmdb.Database, freeClientCap uint64, totalLimit int, clock mclock.Clock, removePeer func(string)) *freeClientPool {
	pool := &freeClientPool{
		db:            db,
		clock:         clock,
...
func (f *freeClientPool) connect(address, id string) bool {
	...
	// check whccmer (linUsage+connectedBias) is smaller than the highest entry in the connected pool
