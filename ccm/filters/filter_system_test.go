...
func (b *testBackend) ChainDb() ccmdb.Database {
	return b.db
}
...
func TestBlockSubscription(t *testing.T) {
	...
	var (
		...
		chain, _    = core.GenerateChain(params.TestChainConfig, genesis, ccmash.NewFaker(), db, 10, func(i int, gen *core.BlockGen) {})
...
// TestPendingTxFilter tests whccmer pending tx filters retrieve all pending transactions that are posted to the event mux.
...
// TestLogFilterCreation test whccmer a given filter criteria makes sense.
...
// TestInvalidLogFilterCreation tests whccmer invalid filter log criteria results in an error
...
// TestLogFilter tests whccmer log filters match the correct logs that are posted to the event feed.
