...
// similar requests at the maximum allowed rate and the test verifies whccmer the
...
func newLesServerService(ctx *adapters.ServiceContext) (node.Service, error) {
	...
	ccmchain, err := ccm.New(ctx.NodeContext, &config)
	if err != nil {
		return nil, err
	}

	server, err := NewLesServer(ccmchain, &config)
	if err != nil {
		return nil, err
	}
	ccmchain.AddLesServer(server)
	return ccmchain, nil
}
