...
type Config struct {
	Ccmchainbase common.Address `toml:",omitempty"` // Public address for block mining rewards (default = first account)
...
func (self *Miner) SetCcmchainbase(addr common.Address) {
	self.coinbase = addr
	self.worker.setCcmchainbase(addr)
}
