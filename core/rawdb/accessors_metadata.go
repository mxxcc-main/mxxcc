...
func ReadDatabaseVersion(db ccmdb.KeyValueReader) *uint64 {
	var version uint64
...
func WriteDatabaseVersion(db ccmdb.KeyValueWriter, version uint64) {
...
func ReadChainConfig(db ccmdb.KeyValueReader, hash common.Hash) *params.ChainConfig {
...
func WriteChainConfig(db ccmdb.KeyValueWriter, hash common.Hash, cfg *params.ChainConfig) {
	if cfg == nil {
		return
	}
...
func ReadPreimage(db ccmdb.KeyValueReader, hash common.Hash) []byte {
	data, _ := db.Get(preimageKey(hash))
	return data
}

// WritePreimages writes the provided set of preimages to the database.
func WritePreimages(db ccmdb.KeyValueWriter, preimages map[common.Hash][]byte) {
